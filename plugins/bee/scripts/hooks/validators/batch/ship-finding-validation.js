#!/usr/bin/env node
// ship-finding-validation.js — batch validator for /bee:ship Step 3b.8
// (finding-validator parallel spawn block at ship.md:593-599). Dispatches N
// invocations of audit-finding-validator (variable count up to 10 findings per
// batch per ship.md:593), aggregates the per-agent verdicts, and emits a
// single batch verdict.
//
// Per-agent set (BATCH_PER_INSERTION_AGENT_ROSTER['ship-finding-validation.js']):
//   audit-finding-validator — N invocations, one per pending finding, capped
//   at 10 per batch by the conductor. Every agent_outputs[i].agent slug is
//   audit-finding-validator; all route to the same validator filename.
//
